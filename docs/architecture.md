...
- **Grafana user passwords** — generated by `bootstrap_grafana.py`, printed
  once, never stored.
- **Database credentials** — stay on the monitored host, in root-only files
  under `/opt/grafana-prometheus-loki-agent/`. Alloy reads them as secrets, so they
  appear in neither its config nor its logs, and the logs matter because they
  are shipped to Loki, where the client can read them. See
