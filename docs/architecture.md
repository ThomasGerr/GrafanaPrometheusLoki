...
The agent is configured by environment variables alone. Its image
(`agent/Dockerfile`) is Alloy with the config baked in and a start-up script,
`agent/entrypoint.sh`, that generates the per-host part (databases, CrowdSec)
from those variables at every start. So it deploys like any compose app,
Dokploy included, and a changed variable is applied by recreating the
container. `agent/install.sh` only writes the variables and checks the
result. See [agent.md](agent.md).
...
- **Grafana user passwords** — generated by `bootstrap_grafana.py`, printed
  once, never stored.
- **Database credentials** — stay on the monitored host, in the agent's
  environment (a root-only `.env`, or Dokploy's Environment tab). At start-up
  the agent writes them to files inside its container that Alloy reads as
  secrets, so they appear in neither its config nor its logs. The logs matter
  because they are shipped to Loki, where the client can read them. See
